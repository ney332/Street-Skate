            ...
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 14) {
                        ...
                        SummaryMetric(
                            value: "spot.distance.kilometers".localized(session.distanceKm), label: "Distance",
                            icon: "figure.skating", color: Color(hex: "#4CAF50"), delay: 0.6
                        )
                        SummaryMetric(
                            value: "summary.calories".localized(Int(session.calories)), label: "Calories",
                            icon: "flame.fill", color: Color(hex: "#FF6B35"), delay: 0.7
                        )
                        SummaryMetric(
                            value: "\(session.pushCount)", label: "Push",
