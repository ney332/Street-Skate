...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onChange(of: appState.achievementManager.newlyUnlocked) { _, achievement in
                if achievement != nil {
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    Text("Level Progress")
                        ...
                        .foregroundColor(Color.white.opacity(0.5))
                    Spacer()
                    Text("\(user?.xp ?? 0) / \(((user?.xp ?? 0) / 1000 + 1) * 1000) XP")
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundColor(Color(hex: "#FFD700"))
                }
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        ...
                        RoundedRectangle(cornerRadius: 6)
                            ...
                            .frame(width: geo.size.width * (user?.levelProgress ?? 0), height: 10)
